...
$(document).ready(function(){
	$('.nav-tabs li').click(function(e){
		...
		if( $(this).attr('class') == 'head' ) return 0;
		
		var index = $(this).index();
		if( $(this).parent().find( '.head' ).length != 0 ){
				index = index -1 ;
		}
		
		$('.nav-tabs li').removeClass('active');
		$(this).addClass('active').parent().next().find('div').hide().removeClass('active').eq( index ).show().addClass('active');
	}).parent().each(function(){
		$(this).next().find('div').first().show().addClass('active');
	});
	
	$('.show-code a').click(function(e){
		e.preventDefault();
